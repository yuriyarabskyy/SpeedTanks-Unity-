...
public class Play : MonoBehaviour {
	...
	public GameObject fader,cam,pause,score1,score2,exit,next;
	bool flag = false;
	...
	void Update(){
		...
		//if(collider2D.OverlapPoint(cam.camera.ScreenToWorldPoint(Input.mousePosition))&&Input.GetButtonDown("Fire1")&&!flag){
		//	flag = true;
...
//		}	
		if (flag) {
			...
			if(spriteOpacity==0&&GameObject.FindGameObjectsWithTag("Player").Length == 2){
				Instantiate(pause);
				Destroy(gameObject);
			}
		}
